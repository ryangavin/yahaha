...
class UiStore {
  ...
  toggleDrawer(d: 'parts' | 'mixer' | 'settings' | 'charts' | 'regist' | 'looper' | 'multipad' | 'harmony') {
    const open = !this[d]
    this.parts = this.mixer = this.settings = this.charts = this.regist = this.looper = this.multipad = this.harmony = false
    this[d] = open
  }
