...
//   ?open=browser|settings|parts|mixer|multipad|regist|playlist   open an overlay or drawer
...
export function applyUrlParams(search = location.search) {
  ...
  if (open === 'settings' || open === 'parts' || open === 'mixer' || open === 'multipad' || open === 'regist') ui.toggleDrawer(open)
  if (open === 'playlist') {
