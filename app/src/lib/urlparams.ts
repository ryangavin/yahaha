...
//   ?open=browser|settings|parts|mixer|looper|multipad|harmony|regist|playlist|sound   open an overlay or drawer
...
export function applyUrlParams(search = location.search) {
  ...
  if (open === 'settings' || open === 'parts' || open === 'mixer' || open === 'looper' || open === 'multipad' || open === 'harmony' || open === 'regist' || open === 'sound') ui.toggleDrawer(open)
  if (open === 'playlist') {
