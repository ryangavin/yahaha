...
//   ?open=browser|settings|parts|mixer|charts|multipad   open an overlay or drawer
...
export function applyUrlParams(search = location.search) {
  ...
  if (open === 'settings' || open === 'parts' || open === 'mixer' || open === 'charts' || open === 'multipad') ui.toggleDrawer(open)
