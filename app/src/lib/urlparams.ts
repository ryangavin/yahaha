...
//   ?open=browser|settings|parts|mixer|looper|multipad|harmony|regist|playlist   open an overlay or drawer
...
export function applyUrlParams(search = location.search) {
  ...
  if (open === 'settings' || open === 'parts' || open === 'mixer' || open === 'looper' || open === 'multipad' || open === 'harmony' || open === 'regist') ui.toggleDrawer(open)
  if (open === 'playlist') {
