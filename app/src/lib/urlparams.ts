...
//   ?open=browser|settings|parts|mixer|multipad|harmony   open an overlay or drawer
...
export function applyUrlParams(search = location.search) {
  ...
  if (open === 'settings' || open === 'parts' || open === 'mixer' || open === 'multipad' || open === 'harmony') ui.toggleDrawer(open)
