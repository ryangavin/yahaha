...
- **Mixer:** faders 1–8 are the band's eight part volumes: each fader value is that part's CC 7, sent unchanged to the `yahaha` port and the built-in synth. Loading a style sets the faders to the style's own levels (100 where it sets none). Section changes keep the levels you set; changing style resets them. The Launchkey faders use soft takeover: after a style load moves a level, the hardware fader does nothing until it reaches that level (`↕` on screen until then). The master fader sets the synth's output level (100 = unity).
