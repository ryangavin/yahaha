...
- **Genos behaviour** is paraphrased. Refs: `OM`/`RM`/`DL` + printed page (Genos2
  Owner's Manual, Reference Manual, Genos Data List), `[note#Vn MM:SS]` for a video
  listed in that topic note's Sources table. *video-only* = not found in the
...
|---|---|---|---|
| 1 | [transport/acmp-switch](#chord-following) | No drums-only playing with a full-keyboard voice, no "band out" press mid-song, Sync Start needs a chord | P1 |
...
Close behind: tempo reset/auto-repeat (P1, tiny), Multi Pads on the Launchkey (P1),
AI Fingered keeping the lowest note as bass (P1 if a Genos confirms it), fill
late-press allowance (P1 if a Genos has one).
...
| tempo/reset-repeat | TEMPO − and + together restore the style's tempo; holding a button repeats. OM p.46 | No reset, no auto-repeat, 2 BPM per press | Reset and repeat missing; step size unconfirmed | P1 | `resetTempo` (Launchkey > + Function together) and auto-repeat while held. [note](notes/tempo-transpose.md) |
...
| tempo/change-behavior | Tempo Lock/Hold/Reset; assignable toggles pop up the new value. RM p.12, p.144; [tempo V2 still 00:30] | Implemented (#26), default Hold; toggles are API-only | Not assignable | P2 | Two `controllers::FUNCTIONS` rows + a toast |
...
| tempo/param-lock | Groups such as Split Point and Fingering Type can be locked. DL chart | Split Point and Fingering Type (`src/api/param_lock.rs`) | none | – | – |
...
| sections/ending-rit | Second press of the playing Ending slows it gradually. OM p.66 | Linear to 65% (`src/engine/ritardando.rs`); Ending I is a short hit in the style data (#129) | none; README "Known gaps" still lists it as not done | P2 | Fix README Known gaps |
...
| sections/auto-fill | Every Main press plays a fill first; System setting, not in Registration. OM p.67, RM p.12, DL p.82 | Matches; default on (PSR default off [auto-fill V1 00:53]) | Factory default unknown | P2 | Owner question |
...
| Concept | Genos behaviour | yahaha today | Gap | Pri | Next step |
|---|---|---|---|---|---|
| transport/acmp-switch | ACMP off: START plays rhythm only, the whole keyboard plays the Right voices, Sync Start fires on any key; OTS and Looper REC turn ACMP on. OM p.44, p.47, p.66, p.68 | No ACMP switch (`src/engine/multipad.rs`, `src/controllers.rs`); rhythm-only lasts until the first chord; Sync Start needs a chord | No drums-only play, no "band out" press, no any-key Sync Start | P1 | `Button::Acmp` (default on; registrable; OTS/REC force on), key routing, any-key Sync Start, pad + assignable. [note](notes/sync-start-stop.md) |
| chord/fingering-types | Seven types plus Fingered\* in Upper. RM p.9, OM p.46 | All built (`src/fingering.rs`); default Fingered On Bass | Genos factory default unknown | P2 | Owner question |
| chord/ai-fingered-bass | AI Fingered is "basically Fingered" (root bass) per RM p.9; Keyboard-Akademie videos are about slash-bass with AI Fingered (*pending video*) | Forces root bass | Possible loss of slash chords for AI Fingered players | P1 if confirmed | Read W2phsGwMgoA / 71G1T761qtA; if confirmed, keep the lowest note for complete chords. [note](notes/fingering.md) |
...
| split/three-points | Style, Left and Right 3 split points with ordering rules; set by holding a label and pressing a key. OM p.49–50, DL p.88 | One split point; no Right 3 split; no key capture | Missing | P2 | `split_r3` + routing, then separate Style/Left, app "set from key" |
| split/left-hold | LEFT HOLD sustains the Left voice after release; cleared by stopping the style; in Registration. OM p.49, RM p.141, DL p.82 | Not built (#31 remainder) | Missing | P1 | Left Hold flag + assignable + registrable + pad. [note](notes/split-detection.md) |
| split/upper-manual-bass | Upper forces Fingered\*; Manual Bass moves the Style bass voice to Left. OM p.51 | Built | none | – | – |
| sync/sync-start | Standby; pressed while playing it stops and re-arms; OTS turns it on. OM p.66, p.47 | Matches | none | – | – |
...
| Concept | Genos behaviour | yahaha today | Gap | Pri | Next step |
|---|---|---|---|---|---|
| ots/recall-contents | An OTS recalls voice and on/off plus each part's mixer values (pan, reverb/chorus, filter, EQ), Voice Edit, and turns ACMP and Sync Start on. OM p.47, DL p.82–92; [ots V2 01:05] | Voice (via the program map), on/off, volume, octave; arms Sync Start. `parse_ots` drops pan, sends, filter, portamento, bend range and XG part SysEx (`src/sff.rs`, `src/parts.rs`) | OTS pan and effects not applied | P1 | Keep the per-part controllers in `parse_ots` and send them on recall; corpus test. [note](notes/ots.md) |
...
| control/fader-catch | Slider catch mode; a reviewer wants a jump option [style-feel V2 23:37] | Soft takeover within 2 | Same design | P2 | Optional "jump" mode |
...
| feel/voice-change-cutoff | Genos2 cuts held notes on OTS/variation voice changes; players lift their hands [style-feel V2 25:58–27:29, video-only] | Plugin swap sends All Notes Off; SF2 behaviour unverified | A chance to beat the Genos | P2 | Test held notes across OTS Link; keep the outgoing voice until its keys are released |
...
| feel/smart-chord | SX920 only: key + mode, one finger plays the diatonic chord [style-feel V1 00:42–06:06] | None | Competitive gap, not parity | P2 | Owner question |
...
| Concept | Genos behaviour | yahaha today | Gap | Pri | Next step |
|---|---|---|---|---|---|
| pads/playback | Start at once stopped / next bar playing; press again restarts; up to 4; Repeat in step. OM p.74, RM p.65 | Matches (`docs/multipad.md`) | none (*pending video*) | – | – |
...
| pads/chord-match | Pads follow the chord; CM7-based phrases. OM p.74, RM p.65 | Matches | none | – | – |
...
| pads/level | Panel Balance level for all pads; in Registration/OTS. OM p.90 | Per-pad CC7 only | No overall pad level | P2 | With mixer/style-balance |
| pads/ots-bank | Each OTS names a pad bank. OM p.60, DL; [ots V2 01:05] | Not recalled | Missing | P2 | With ots/harmony-pads |
...
| harmony/storage | Type and settings in Voice Set, OTS, Registration. DL chart | Registration yes; OTS and Voice Set no | Missing | P2 | With ots/harmony-pads |
...
| songs/song-plus-style | MIDI Song + Style: Style replaces ch 9–16. RM p.75, p.78 | No Song player (out of scope) | Owner decision | – | Owner question |
