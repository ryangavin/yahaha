...
| Registration | Registration Bank +, Registration Bank − (the REGIST BANK [+]/[−] buttons: they load the next or previous bank file, see docs/registration.md); Registration Memory 1–10 (the buttons: with MEMORY armed they memorize), Registration Memory (the MEMORY button), Registration Freeze On/Off, Registration Sequence On/Off (RM p.141). **Regist +, Regist −**: the Genos gives a pedal these through Pedal Control on the Registration Sequence display (RM p.114); yahaha has no such page, so they are rows of their own. They step the sequence while it is on and programmed, else the bank's stored buttons in order (docs/registration.md, Decisions). |
