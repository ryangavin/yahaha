...
gh pr list -R ryangavin/yahaha --base integration/m3-ui --state open --limit 50 --json number,title,labels,headRefOid,mergeable,mergeStateStatus,headRefName --jq '.[] | "\(.number)\t\(.mergeable)\t\(.mergeStateStatus)\t[\([.labels[].name]|join(","))]\t\(.headRefOid[0:10])\t\(.headRefName)\t\(.title)"'
for n in $(gh pr list -R ryangavin/yahaha --base integration/m3-ui --state open --label ready-to-merge --json number --jq '.[].number'); do
