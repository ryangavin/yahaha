...
You are the FINAL gate for one PR before the coordinator merges it into `integration/m3-ui`. The PR's last fix commits were pushed after its last adversarial review, so no one has reviewed them yet. You review them adversarially. If you find a blocker, you fix it yourself: this is the last round, so there is no hand-back.
...
4. Merge `origin/integration/m3-ui` into the branch with a MERGE COMMIT, not a rebase. Resolve conflicts keeping both sides, and check the semantic clashes with what landed recently:
