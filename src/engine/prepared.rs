...
impl Prepared {
    ...
    /// Every voice the style sends each Style part (channels 9-16): its channel setup's and
    /// every program change in its sections, as (channel 0-based, bank MSB, bank LSB,
    ...
    pub fn program_changes(&self) -> Vec<(u8, u8, u8, u8)> {
        ...
        let setup = bank;
        for sec in self.sections.iter().flatten() {
            let mut bank = setup;
            for e in &sec.events {
                let Some(d) = sec.rules[e.src as usize & 15].as_ref().map(|r| r.dest_ch as usize & 15) else { continue };
                match e.kind {
